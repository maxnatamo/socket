...
sck_http_single_header_t *sck_http_header_parse_single(char *header_c) {
    sck_http_single_header_t *header = malloc(sizeof(sck_http_single_header_t));
...
sck_http_headers_t *sck_http_header_parse(char *raw_header) {
    ...
    http_headers->single_headers     = malloc(sizeof(sck_http_single_header_t) * lines->length);
    ...
    for(int i = 1; i < lines->length - 1; i++) {
        http_headers->single_headers[i] = sck_http_header_parse_single(lines->data[i]);
    }
